# -*- coding: utf-8 -*-
"""
滚动回测策略（5分钟级别）：LightGBM回归信号 + 因子标准化
- 33个币种：BTC, ETH, XRP, BNB, SOL, DOGE, TRX, ADA, LINK, AVAX, DOT, MATIC等
- 16个滚动周期：2022-01-01到2025-10-31，每个周期3个月
- 训练集：累积历史数据到验证集开始前
- 验证集：回测前6个月
- 测试集：3个月回测期
- 数据频率：5分钟（因子周期相应调整为1小时的12倍）
- 模型：LightGBM（高效梯度提升树，支持GPU加速）
- 标准化：100期滚动窗口Z-score标准化（与因子计算一致）
"""
...
SYMBOLS = [
    ...
    # "LUNA","SHIB","PEPE","ZEC","PUMP","ENA","SUI","TAO","VIRTUAL"
]
...
# 测试模式：仅运行第4个周期（2022-10-01 to 2022-12-31）
TEST_SINGLE_PERIOD = 4  # 设置为4测试第四个周期，None为运行全部周期
...
# 缓存目录（复用5分钟因子缓存，添加_norm后缀区分标准化版本）
CACHE_DIR = "cache_5m_norm"
RESULTS_DIR = "rolling_results_5m_regression_lgb_norm"
...
# 模型保存目录（LightGBM标准化版本专用）
MODELS_DIR = "trained_models_5m_regression_lgb_norm"
...
def compute_all_indicators(df_ohlcv: pd.DataFrame) -> pd.DataFrame:
    """使用pandas_ta计算所有可用技术指标 + 100期滚动窗口标准化"""
    df = df_ohlcv.copy()
    ...
    # ============================================================
    # 因子配置：使用全部138个pandas_ta因子 + 100期滚动窗口标准化
    # 让LightGBM自动进行特征选择，避免人为过度筛选导致信息损失
    # ============================================================
    causal_strategy = ta.Study(
        name="AllFactors_With_Normalization",
        ta=[
    ...
    # ============ 新增：100期滚动窗口Z-score标准化（与因子计算一致）============
    # 保留原始OHLCV列不变
    ohlcv_cols = ['open', 'high', 'low', 'close', 'volume', 'adj_close']
    indicator_cols = [col for col in df.columns if col not in ohlcv_cols]

    print(f"[标准化] 对 {len(indicator_cols)} 个技术指标应用100周期滚动窗口标准化...")

    for col in indicator_cols:
        series = df[col]
        # Z-score标准化
        mean = series.rolling(100, min_periods=1).mean()
        std = series.rolling(100, min_periods=1).std()
        df[col] = (series - mean) / (std + 1e-10)

    return df


def compute_all_features(df_ohlcv: pd.DataFrame, symbol: str) -> pd.DataFrame:
    """计算所有技术指标特征（仅pandas_ta，已移除附加因子）"""
    # 第一步：计算原有的技术指标（已包含标准化）
    merged = compute_all_indicators(df_ohlcv)

    # 第二步：附加因子已移除（仅使用ICIR筛选的pandas_ta指标）
    # ❌ 移除：merged = additional_features.compute_safe_additional_factors(merged)
    print(f"[{symbol}] 技术指标数: {len(merged.columns)} (仅pandas_ta，已移除附加因子)")
...
def build_feature_matrix(df_all: pd.DataFrame, n: int) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    # 不进行 shift(1)：直接使用刚刚结束的那个bar的特征

    # ❌ 删除重复调用：compute_all_features 已经执行过 ffill().dropna()
    # X = X.ffill().dropna()
...
def calculate_time_weights(index: pd.DatetimeIndex, method: str = TIME_WEIGHT_METHOD,
    ...
    Args:
        index: 时间索引
        method: 权重计算方法 ("exponential", "linear", "sqrt")
        decay_factor: 衰减因子（仅用于exponential方法）
        min_weight: 最小权重
        max_weight: 最大权重

    Returns:
        权重数组，在[min_weight, max_weight]范围内
    """
    if len(index) == 0:
        return np.array([])
    ...
    if method == "exponential":
        # 指数衰减：weight = decay_factor ^ (1 - time_position)
        weights = decay_factor ** (1 - time_positions)
    elif method == "linear":
        # 线性增长：weight = min_weight + (max_weight - min_weight) * time_position
        weights = min_weight + (max_weight - min_weight) * time_positions
    elif method == "sqrt":
        # 平方根增长：weight = min_weight + (max_weight - min_weight) * sqrt(time_position)
    ...
    # 转换为numpy数组并归一化权重，使平均权重为1（保持样本总权重不变）
...
def train_lgb_reg(X_train: pd.DataFrame, y_train: pd.Series, X_val: pd.DataFrame, y_val: pd.Series,
    ...
    params = {
        ...
        'metric': ['rmse', 'mae'],
        'boosting_type': 'gbdt',
        'num_leaves': 127,  # 2^7 - 1，相当于max_depth=7
        'learning_rate': 0.02,
        'feature_fraction': 0.7,  # 相当于colsample_bytree
        'bagging_fraction': 0.7,  # 相当于subsample
        'bagging_freq': 1,
...
def plot_training_curves(model, period_no: int, save_dir: str = TRAINING_CURVES_DIR):
    ...
    try:
        # LightGBM使用不同的API获取训练历史
        evals_result = model.evals_result_

        # LightGBM的键名是'train'和'valid'
        train_key = 'train'
        val_key = 'valid'
...
def print_regression_metrics(y_true: pd.Series, y_pred: np.ndarray, dataset_name: str = "Dataset"):
    ...
    # 计算方向准确率（预测方向与实际方向是否一致）
    y_true_direction = np.sign(y_true)  # 1: 上涨, -1: 下跌, 0: 不变
...
def reg_signal_backtest(reg_pred: pd.Series, px_open: pd.Series,
    ...
    - reg_pred: 回归预测值（未来收益率）
    - 开多条件: reg_pred >= reg_long_thr
    - 开空条件: reg_pred <= reg_short_thr
    - 平多条件: reg_pred <= reg_exit_long_thr（每个bar检查）
    - 平空条件: reg_pred >= reg_exit_short_thr（每个bar检查）
    - max_trades: 最大交易次数限制，超过后停止交易
    - stop_loss_pct: 单笔相对入场价的止损比例（如0.01=1%）
    - n_hold: 保留参数以兼容旧代码，但不再使用
    """
    # 对齐索引
...
def _process_single_symbol_features(symbol: str, period_config: dict) -> dict:
    ...
    cache_file = os.path.join(CACHE_DIR, f"features_plus_{symbol}_rolling_5m_norm.parquet")
...
def run_period_with_shared_model(period_config: dict, use_time_weights: bool = None,
    ...
    print(f"\n[Shared-Model-Regression-LGB-Norm] 周期 {period_no} {period_config['test_start']} -> {period_config['test_end']}")
    ...
    # ❌ 移除 Winsorization：LightGBM 对异常值有鲁棒性，不需要预处理
    # 直接使用原始特征矩阵（已经标准化）
    for sym, d in sym_data.items():
    ...
    # 阈值基于"测试开始前1年"的 pooled 分布（为空则回退验证集）
    test_start_str = period_config['test_start'].strftime('%Y-%m-%d %H:%M:%S')
    ...
    try:
        period_no = period_config['period']
        model_path = os.path.join(MODELS_DIR, f"period_{period_no}_shared_reg_5m_norm.pkl")
        cfg_path = os.path.join(MODELS_DIR, f"period_{period_no}_shared_config_5m_norm.json")
        joblib.dump(reg_model, model_path)
        cfg = {
            "period": int(period_no),
            "model_type": "regression",
            "model": "lightgbm",
            "standardization": "100_period_rolling_zscore",
            "shared": True,
            "symbols": SYMBOLS,
    ...
    # 对每个符号进行测试集预测与回测
    results = []
    test_metrics_list = []  # 存储所有币种的测试集指标
...
def main():
    """主函数：运行所有币种所有周期的滚动回测（5分钟级别，LightGBM回归模型+标准化）"""
    # 清空之前的交易记录
    clear_all_trades()

    print("=" * 80)
    print("开始滚动回测（5分钟级别，LightGBM回归模型+100期滚动标准化）...")
    print("=" * 80)
    print(f"数据频率: 5分钟")
    print(f"预测时间跨度: {N_HORIZON} 个5分钟 = {N_HORIZON * 5 / 60:.1f} 小时")
    print(f"币种数量: {len(SYMBOLS)}")
    print(f"回测周期: {len(BACKTEST_PERIODS)}")
    print(f"总任务数: {len(SYMBOLS) * len(BACKTEST_PERIODS)}")
    print(f"数据目录: {DATA_DIR}")
    print(f"缓存目录: {CACHE_DIR}")
    print(f"结果目录: {RESULTS_DIR}")
    print(f"模型目录: {MODELS_DIR}")
    print(f"模型类型: LightGBM回归")
    print(f"标准化: 100期滚动窗口Z-score")
    print("=" * 80)

    all_results = []
    all_training_metrics = []  # 存储所有周期的训练指标

    if USE_SAME_MODEL:
        # 测试模式：仅运行单个周期
        if TEST_SINGLE_PERIOD is not None:
            if TEST_SINGLE_PERIOD < 1 or TEST_SINGLE_PERIOD > len(BACKTEST_PERIODS):
                print(f"错误：TEST_SINGLE_PERIOD={TEST_SINGLE_PERIOD} 超出范围 [1, {len(BACKTEST_PERIODS)}]")
                return

            period_config = BACKTEST_PERIODS[TEST_SINGLE_PERIOD - 1]
            print(f"\n[测试模式] 仅运行周期 {TEST_SINGLE_PERIOD}: {period_config['test_start']} -> {period_config['test_end']}\n")
