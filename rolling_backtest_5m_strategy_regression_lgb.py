# -*- coding: utf-8 -*-
"""
滚动回测策略（5分钟级别）：LightGBM回归信号
- 33个币种：BTC, ETH, XRP, BNB, SOL, DOGE, TRX, ADA, LINK, AVAX, DOT, MATIC等
- 8个滚动周期：2022-01-01到2025-10-31，每个周期6个月
- 训练集：累积历史数据到验证集开始前
- 验证集：回测前6个月
- 测试集：6个月回测期
...
# 缓存目录（复用5分钟因子缓存）
CACHE_DIR = "cache_5m"
RESULTS_DIR = "rolling_results_5m_regression_lgb"
...
# 模型保存目录（LightGBM专用）
MODELS_DIR = "trained_models_5m_regression_lgb"
...
def run_period_with_shared_model(period_config: dict, use_time_weights: bool = None,
    ...
    print(f"\n[Shared-Model-Regression] 周期 {period_no} {period_config['test_start']} -> {period_config['test_end']}")
    ...
    # ❌ 移除 Winsorization：XGBoost 对异常值有鲁棒性，不需要预处理
    # q_low = X_train_all.quantile(WINSOR_LO)
    # q_high = X_train_all.quantile(WINSOR_HI)
    # X_train_all = X_train_all.clip(lower=q_low, upper=q_high, axis=1)
    # X_val_all = X_val_all.clip(lower=q_low, upper=q_high, axis=1)
    ...
    for sym, d in sym_data.items():
        ...
        X_ref_list.append(d['Xc'].loc[m_ref])  # 不再应用 clip
    ...
    try:
        ...
        cfg = {
            ...
            # ❌ 移除 winsorization 参数（不再使用）
            # "winsor_lo": float(WINSOR_LO),
            # "winsor_hi": float(WINSOR_HI),
            # "q_low": {k: float(v) for k, v in q_low.items()},
            # "q_high": {k: float(v) for k, v in q_high.items()},
...
def main():
    """主函数：运行所有币种所有周期的滚动回测（5分钟级别，回归模型）"""
    # 清空之前的交易记录
    clear_all_trades()

    print("=" * 80)
    print("开始滚动回测（5分钟级别，LightGBM回归模型）...")
    print("=" * 80)
    print(f"数据频率: 5分钟")
